(function () {
      ...
      async function loadProjectsToSidebar() {
        ...
        try {
          ...
          if (currentDb) {
            try {
              const found = items.find(it => (((it.slug || it.name || it.file || '') + '').replace(/\.sqlite$/, '')) === currentDb);
              if (found) try { updateHeaderProjectInfo(currentDb, found.title || currentDb, found.image || ''); } catch(e) {}
            } catch (e) {}
          }
          // Render only projects that are NOT currently selected
          const visibleItems = items.filter(it => (((it.slug || it.name || it.file || '') + '').replace(/\.sqlite$/, '')) !== currentDb);
          if (!visibleItems.length) {
            wrap.innerHTML = '<div class="muted">暂无项目</div>';
            return;
          }
          visibleItems.forEach(it => {
            const dbId = ((it.slug || it.name || it.file || '') + '').replace(/\.sqlite$/, '');
            ...
            if (it.image && typeof it.image === 'string' && it.image.trim()) {
              ...
              const initials = (it.title || dbId || '?').replace(/\s+/g, '').slice(0,2).toUpperCase() || '?';
            ...
            } catch (e) { label.textContent = title; }
            ...
            entry.addEventListener('click', (e) => {
              ...
              entry._clickTimer = setTimeout(() => {
                ...
                try { selectProjectWithAnimation(dbId, entry, title, entry.getAttribute('data-image') || ''); } catch (e) { try { setUrlParam('db', dbId); } catch (err) {} }
              }, 260);
            });
              ...
              if (entry._clickTimer) { clearTimeout(entry._clickTimer); entry._clickTimer = null; }
              // open edit modal
              try {
                try { updateHeaderProjectInfo(dbId, title, entry.getAttribute('data-image') || ''); } catch (e) {}
                openEditProjectModal(dbId, entry.getAttribute('data-title') || '', entry.getAttribute('data-image') || '', entry.getAttribute('data-desc') || '');
              } catch (err) {}
            });
            entry.addEventListener('keydown', (e) => {
              ...
              if (key === 'Enter' || key === 13) {
                e.preventDefault();
                try { selectProjectWithAnimation(dbId, entry, title, entry.getAttribute('data-image') || ''); } catch (err) { try { setUrlParam('db', dbId); } catch(e) {} }
              }
            });
            wrap.appendChild(entry);
          });
          try { updateSidebarLabelMode(); } catch (e) {}
        } catch (err) {
  ...
  function setSidebarCollapsed(collapsed, persist = true, skipFocus = false, instant = false) {
        const sidebarEl = document.getElementById('projectSidebar');
    ...
    if (sidebarEl.classList.contains('animating') && !instant) return;
        ...
        if (wasCollapsed === collapsed) {
          try { sidebarEl.setAttribute('aria-expanded', String(!collapsed)); } catch (e) {}
          if (persist && window.localStorage) { try { localStorage.setItem(COLLAPSE_KEY, collapsed ? '1' : '0'); } catch (e) {} }
          return;
        }

        // If instant requested, temporarily disable CSS transitions/animations
        if (instant) {
          try { sidebarEl.classList.remove('animating'); } catch(e){}
          try { sidebarEl.classList.add('no-sidebar-transition'); } catch(e){}
          try { if (splitEl) splitEl.classList.add('no-sidebar-transition'); } catch(e){}
        } else {
          // Ensure transitions are enabled if a no-sidebar-transition flag was left (e.g., from hover)
          try { sidebarEl.classList.remove('no-sidebar-transition'); } catch(e){}
          try { if (splitEl) splitEl.classList.remove('no-sidebar-transition'); } catch(e){}
        ...
        try {
          ...
          labels.forEach(l => {
            try {
              ...
              const full = (entry && (entry.getAttribute('data-title') || entry.getAttribute('data-db'))) ? (entry.getAttribute('data-title') || entry.getAttribute('data-db')) : (l.title || '');
              if (full) { l.textContent = full; l.title = full; }
            } catch(e){}
          });
        } catch (e) {}
        ...
        try { sidebarEl.classList.remove('hover-expanded'); } catch(e){}
        try { sidebarEl.classList.remove('no-sidebar-transition'); } catch(e){}
        try { if (splitEl) splitEl.classList.remove('no-sidebar-transition'); } catch(e){}
        ...
        try {
          if (!collapsed && !skipFocus) {
              const selected = sidebarEl.querySelector('.project-list .project-entry.selected');
              const firstEntry = sidebarEl.querySelector('.project-list .project-entry');
              ...
              if (target) {
                ...
                try {
                  if (!preferReduced && target.scrollIntoView) target.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' });
                } catch (e) {}
        ...
        if (!instant) {
          const onTransitionEnd = (evt) => {
            const isSidebarWidth = (evt.target === sidebarEl && (evt.propertyName === 'width' || evt.propertyName === 'padding'));
            const isGridCols = (evt.target === splitEl && evt.propertyName === 'grid-template-columns');
            if (isSidebarWidth || isGridCols) {
              sidebarEl.classList.remove('animating');
              if (splitEl) splitEl.classList.remove('animating');
              try { sidebarEl.removeEventListener('transitionend', onTransitionEnd); } catch(e){}
              try { if (splitEl) splitEl.removeEventListener('transitionend', onTransitionEnd); } catch(e){}
            }
          };
          try { sidebarEl.addEventListener('transitionend', onTransitionEnd); } catch(e){}
          try { if (splitEl) splitEl.addEventListener('transitionend', onTransitionEnd); } catch(e){}
      ...
      async function loadUsersToSidebar() {
        ...
        try {
          ...
          try {
            const current = (window && window.authUser && window.authUser.username) ? (window.authUser.username || '').toString().toLowerCase() : '';
            if (current) {
              items = items.filter(it => ((it.username || '') + '').toString().toLowerCase() !== current);
            }
          } catch (e) {}
          wrap.innerHTML = '';
          if (!items.length) { wrap.innerHTML = '<div class="muted">暂无用户</div>'; return; }
          items.forEach(it => {
            ...
            if (it.avatar && typeof it.avatar === 'string' && it.avatar.trim()) {
              ...
              const initials = (it.displayName || username || '?').toString().replace(/\s+/g,'').slice(0,2).toUpperCase() || '?';
              const span = document.createElement('span');
              span.className = 'project-initials';
              span.textContent = initials;
              entry.appendChild(span);
            ...
            entry.addEventListener('dblclick', (e) => {
              try { e.preventDefault(); const chatInput = document.getElementById('chat-q'); if (chatInput) { chatInput.value = '@' + (username || '') + ' '; try { chatInput.focus(); } catch(e){} } } catch(e){}
            });
            wrap.appendChild(entry);
          });
          try { updateUserSidebarLabelMode(); } catch (e) {}
        } catch (e) {
      ...
      function updateUserSidebarLabelMode() {
        try {
          ...
          let override = null;
          try {
            if (typeof window !== 'undefined' && typeof window.SIDEBAR_LABEL_COMPACT_THRESHOLD !== 'undefined') {
          ...
          try {
            if (override === null && window && window.localStorage) {
              const stored = localStorage.getItem('kbUserSidebarLabelCompactThreshold');
              if (stored !== null) {
          ...
          if (override !== null && !isNaN(override)) {
            ...
            const avail = wrap.clientHeight || wrap.getBoundingClientRect().height || window.innerHeight * 0.5;
            let per = 56;
            if (entries.length >= 2) {
              try { per = Math.max(24, entries[1].offsetTop - entries[0].offsetTop); } catch (e) {}
            } else if (entries.length === 1) {
              try { per = Math.max(24, entries[0].offsetHeight); } catch (e) {}
            }
          ...
          if (enableCompact) {
            ...
            entries.forEach(ent => {
              try {
                ...
                if (lab) {
                  lab.textContent = lab.dataset.short || (lab.dataset.full || lab.textContent);
        ...
        } catch (e) { console.warn('updateUserSidebarLabelMode failed', e); }
      }
      ...
      function setupUserSidebarHover() {
        try {
          ...
          projectListEl.addEventListener('mouseover', (e) => {
            try {
              const entry = e.target && e.target.closest ? e.target.closest('.project-entry') : null;
              if (!entry) return;
              if (_sidebarHoverTimer) { clearTimeout(_sidebarHoverTimer); _sidebarHoverTimer = null; }
              try { sidebar.classList.add('hover-expanded'); } catch (err) {}
              try { sidebar.classList.add('no-sidebar-transition'); if (split) split.classList.add('no-sidebar-transition'); } catch (err) {}
              try {
                ...
                if (lab && lab.dataset && lab.dataset.full) {
                  try {
                    if (sidebar.classList.contains('compact-labels')) {
                      const others = sidebar.querySelectorAll('.project-entry-label');
                      others.forEach(o => { if (o !== lab && o.dataset && o.dataset.short) { try { o.textContent = o.dataset.short; } catch(e){} } });
                    }
                  } catch(e) {}
          ...
          projectListEl.addEventListener('mouseout', (e) => {
            try {
              ...
              const enteringEntry = to && to.closest ? to.closest('.project-entry') : null;
              ...
              _sidebarHoverTimer = setTimeout(() => {
                ...
                if (sidebar.classList.contains('hover-expanded')) {
                  try { sidebar.classList.remove('hover-expanded'); } catch (err) {}
                  try { sidebar.classList.remove('no-sidebar-transition'); if (split) split.classList.remove('no-sidebar-transition'); } catch (err) {}
                }
                try { updateUserSidebarLabelMode(); } catch (err) {}
              }, 160);
            } catch (err) {}
          });
          ...
          try {
            ...
            window.addEventListener('resize', () => {
              ...
              _lblResizeTimer = setTimeout(() => { try { updateUserSidebarLabelMode(); } catch(e) {} }, 160);
            });
            if (typeof MutationObserver !== 'undefined') {
              const mo = new MutationObserver(() => { try { updateUserSidebarLabelMode(); } catch(e) {} });
              try { mo.observe(projectListEl, { childList: true }); } catch (e) {}
            }
          } catch (e) {}
        } catch (e) {}
      }
  ...
  const editProjectPreviewImg = document.getElementById('editProjectPreviewImg');
  const editProjectPreviewStatus = document.getElementById('editProjectPreviewStatus');
    // Create project modal preview elements
    const inputProjectCreateImageUrl = document.getElementById('inputProjectCreateImageUrl');
    const createProjectPreview = document.getElementById('createProjectPreview');
    const createProjectPreviewImg = document.getElementById('createProjectPreviewImg');
    const createProjectPreviewStatus = document.getElementById('createProjectPreviewStatus');
      ...
      const inputProjectEditTitle = document.getElementById('inputProjectEditTitle');
      ...
      function showToast(msg, type = 'success', duration = 3000) {
        try {
          ...
          el.className = 'kb-toast kb-toast-' + (type === 'error' ? 'error' : 'success');
          el.textContent = msg;
          wrap.appendChild(el);
          // show
      ...
      function updateProjectEntryInUI(dbId, title, desc, image) {
        try {
          const sel = document.querySelector('.project-list .project-entry[data-db="' + dbId + '"]');
          if (sel) {
            ...
            if (labelEl) {
              try {
                ...
                const sidebarEl = document.getElementById('projectSidebar');
                const compact = sidebarEl && sidebarEl.classList.contains('compact-labels');
                labelEl.textContent = compact ? (labelEl.dataset.short || base) : base;
                labelEl.title = base;
              } catch (e) { labelEl.textContent = title || dbId; }
            }
            // update avatar
            if (image && image.trim()) {
              ...
              if (imgSpan) {
                ...
                const initials = sel.querySelector('.project-initials'); if (initials) initials.remove();
                const img = document.createElement('span');
                img.className = 'project-avatar-img';
              ...
              const imgSpan = sel.querySelector('.project-avatar-img'); if (imgSpan) imgSpan.remove();
              const initialsText = (title || dbId || '?').replace(/\s+/g, '').slice(0,2).toUpperCase() || '?';
              const span = document.createElement('span'); span.className = 'project-initials'; span.textContent = initialsText;
              const existing = sel.querySelector('.project-initials'); if (!existing) sel.insertBefore(span, sel.firstChild);
            }
          }
          // if this is the currently selected DB, update header as well
          try { if ((getUrlParam('db') || '') === dbId) updateHeaderProjectInfo(dbId, title || dbId, image || ''); } catch(e) {}
          // update card in project selection modal if present
          const card = document.querySelector('#dbProjectListWrap .db-project-card[data-db="' + dbId + '"]');
          if (card) {
            const titleEl = card.querySelector('.db-project-title');
            if (titleEl) { titleEl.textContent = title || dbId; titleEl.title = title || dbId; card.dataset.title = title || dbId; }
            const descEl = card.querySelector('.db-project-desc');
            if (descEl) { descEl.textContent = desc || ''; card.dataset.desc = desc || ''; }
            ...
            if (avatar) {
              if (image && image.trim()) {
                ...
                avatar.textContent = (title || dbId || '').replace(/\s+/g,'').slice(0,2).toUpperCase() || '?';
              }
            }
          }
        } catch (e) { console.warn('updateProjectEntryInUI failed', e); }
      }
      ...
      function updateHeaderProjectInfo(dbId, title, image) {
        try {
          ...
          try { avatar.textContent = ''; } catch (e) {}
          if (image && image.trim()) {
            avatar.style.backgroundImage = `url(${image.trim()})`;
          } else {
            const initials = (safeTitle || dbId || '?').toString().replace(/\s+/g, '').slice(0,2).toUpperCase() || '?';
            avatar.style.backgroundImage = '';
            avatar.textContent = initials;
          }
          wrap.style.display = 'flex';
        } catch (e) { console.warn('updateHeaderProjectInfo failed', e); }
      }
      ...
      function updateSidebarLabelMode() {
        ...
        } catch (e) { console.warn('updateSidebarLabelMode failed', e); }
      }
      ...
      function playHeaderAppearAnimation() {
        try {
          ...
          const onEnd = () => { headerWrap.removeEventListener('animationend', onEnd); headerWrap.classList.remove('anim-appear'); };
          headerWrap.addEventListener('animationend', onEnd);
        } catch (e) { console.warn('playHeaderAppearAnimation failed', e); }
      }
      ...
      function selectProjectWithAnimation(dbId, entryEl, title, image) {
        try {
          if (!dbId) return;
          if (!entryEl) { updateHeaderProjectInfo(dbId, title, image); setUrlParam('db', dbId); return; }
          ...
          try { updateHeaderProjectInfo(dbId, title, image); } catch (e) {}
          try { playHeaderAppearAnimation(); } catch (e) {}
          // hide sidebar for a cleaner UX after selecting a project
          try { setSidebarCollapsed(true, false); } catch (e) {}
          const onEnd = () => {
            entryEl.removeEventListener('animationend', onEnd);
            try { entryEl.remove(); } catch (e) {}
            ...
            try {
              if (prevDb && prevDb !== dbId) {
                const wrap = document.querySelector('.project-list');
                if (wrap && !wrap.querySelector('.project-entry[data-db="' + prevDb + '"]')) {
                  const entry = document.createElement('a');
                  entry.className = 'project-entry';
                  entry.href = '#';
                  ...
                  if (prevImage) {
                    ...
                    const initials = (displayTitle || prevDb || '?').replace(/\s+/g, '').slice(0,2).toUpperCase() || '?';
                    const span = document.createElement('span');
                    span.className = 'project-initials';
                    span.textContent = initials;
                    entry.appendChild(span);
                  ...
                  try {
                    ...
                    const isCollapsed = sidebarEl && sidebarEl.classList.contains('collapsed');
                    label.setAttribute('aria-hidden', String(!!isCollapsed));
                  } catch (e) { label.textContent = displayTitle; }
                  entry.appendChild(label);
                  // attach handlers
                  entry.addEventListener('click', (e) => {
                    e.preventDefault(); e.stopPropagation();
                    if (entry._clickTimer) return;
                    entry._clickTimer = setTimeout(() => { entry._clickTimer = null; try { selectProjectWithAnimation(prevDb, entry, displayTitle, entry.getAttribute('data-image') || ''); } catch (err) { try { setUrlParam('db', prevDb); } catch(e){} } }, 260);
                  });
                  entry.addEventListener('dblclick', (e) => { e.preventDefault(); e.stopPropagation(); if (entry._clickTimer) { clearTimeout(entry._clickTimer); entry._clickTimer = null; } try { updateHeaderProjectInfo(prevDb, displayTitle, entry.getAttribute('data-image') || ''); } catch (e) {} openEditProjectModal(prevDb, entry.getAttribute('data-title') || '', entry.getAttribute('data-image') || '', entry.getAttribute('data-desc') || ''); });
                  entry.addEventListener('keydown', (e) => { const key = e.key || e.keyCode; if (key === 'Enter' || key === 13) { e.preventDefault(); try { selectProjectWithAnimation(prevDb, entry, displayTitle, entry.getAttribute('data-image') || ''); } catch (err) { try { setUrlParam('db', prevDb); } catch(e) {} } } });
                  try { wrap.insertBefore(entry, wrap.firstChild); } catch (e) { wrap.appendChild(entry); }
                  try { updateSidebarLabelMode(); } catch (e) {}
                }
              }
            } catch (e) { }
            try { setUrlParam('db', dbId); } catch (e) {}
        ...
        } catch (e) { console.warn('selectProjectWithAnimation failed', e); }
      }
      ...
      function openEditProjectModal(dbId, title, image, desc) {
        ...
        // update preview
        try { updateEditProjectPreview(image || ''); } catch (e) {}
        if (editProjectError) editProjectError.style.display = 'none';
        editProjectModal.dataset.db = dbId;
        try { (inputProjectEditTitle || inputProjectImageUrl) && (inputProjectEditTitle || inputProjectImageUrl).focus(); } catch(e){}
      }

      function updateEditProjectPreview(url) {
        if (!editProjectPreview || !editProjectPreviewImg || !editProjectPreviewStatus) return;
      ...
      function updateCreateProjectPreview(url) {
        if (!createProjectPreview || !createProjectPreviewImg || !createProjectPreviewStatus) return;
        const trimmed = (typeof url === 'string' ? url.trim() : '');
        if (!trimmed) {
      ...
      if (inputProjectCreateImageUrl) {
        inputProjectCreateImageUrl.addEventListener('input', (e) => {
          try { updateCreateProjectPreview(e.target.value || ''); } catch (err) {}
        });
      }

      if (inputProjectImageUrl) {
        inputProjectImageUrl.addEventListener('input', (e) => {
          try { updateEditProjectPreview(e.target.value || ''); } catch (err) {}
        });
      }

      if (btnCancelEditProject) btnCancelEditProject.addEventListener('click', () => { if (editProjectModal) editProjectModal.style.display='none'; });

      if (btnSubmitEditProject) btnSubmitEditProject.addEventListener('click', async () => {
        if (!editProjectModal) return;
        const dbId = editProjectModal.dataset.db;
        if (!dbId) return;
        const url = inputProjectImageUrl ? inputProjectImageUrl.value.trim() : '';
        const title = inputProjectEditTitle ? inputProjectEditTitle.value.trim() : '';
        const desc = inputProjectEditDesc ? inputProjectEditDesc.value.trim() : '';
        ...
        try {
          ...
          const resp = await fetch('/api/kb/update_project', {
            ...
            body: JSON.stringify({ name: dbId, image: url, title: title, description: desc })
          });
          // Some servers may return non-JSON (e.g., 404 Not Found with text), so parse safely
          const respText = await resp.text();
          let json = null;
          try { json = respText ? JSON.parse(respText) : null; } catch (e) { json = null; }
          if (!resp.ok) {
            const msg = (json && json.message) ? json.message : (respText || `HTTP ${resp.status}`);
            throw new Error(msg);
          }
          if (!json || !(json.success || json.ok)) {
            const msg = (json && json.message) ? json.message : (respText || '保存失败');
            throw new Error(msg);
          }
          if (editProjectModal) editProjectModal.style.display = 'none';
          // update UI immediately
          try { updateProjectEntryInUI(dbId, title, desc, url); } catch(e){}
          // show toast
          try { showToast('已保存', 'success'); } catch(e){}
          // refresh project-selection modal if it's visible
          try { const modal = document.getElementById('dbSelectModal'); if (modal && modal.style.display && modal.style.display !== 'none' && typeof loadProjectListToModal === 'function') loadProjectListToModal(getUrlParam('db')); } catch (e) {}
        } catch (err) {
          if (editProjectError) { editProjectError.textContent = '保存失败：' + (err && err.message ? err.message : err); editProjectError.style.display = 'block'; }
          try { showToast('保存失败：' + (err && err.message ? err.message : ''), 'error', 4000); } catch (e) {}
          console.warn('update project failed', err);
        } finally {
          try { btnSubmitEditProject.disabled = false; } catch(e){}
        }
      });

      document.addEventListener('DOMContentLoaded', () => {
        // default to collapsed immediately to avoid flicker
        try { (window.setSidebarCollapsed || setSidebarCollapsed)(true, false); } catch (e) {}
        // initialize sidebar projects
        try { loadProjectsToSidebar(); } catch (e) {}
        // initialize right-side user sidebar
        try { loadUsersToSidebar(); } catch (e) {}
        try { setupUserSidebarHover(); } catch (e) {}
        const btnRefreshUsers = document.getElementById('btnRefreshUsers');
        if (btnRefreshUsers) {
          btnRefreshUsers.addEventListener('click', (e) => { e.preventDefault(); try { loadUsersToSidebar(); } catch(err){} });
        }
        const btnMoreUsers = document.getElementById('btnMoreUsers');
        if (btnMoreUsers) {
          btnMoreUsers.addEventListener('click', (e) => { e.preventDefault(); try { showToast('暂无更多用户', 'info'); } catch(err){} });
        }
        const btn = document.getElementById('btnCreateProjectSidebar');
        if (btn) {
          btn.addEventListener('click', (e) => {
            e.preventDefault();
            try { openCreateProjectModal(); } catch (err) {}
          });
        }
        ...
        if (more) {
          more.addEventListener('click', (e) => {
            e.preventDefault();
            try { showDbSelectModal(true); } catch (err) {}
          });
        }
        ...
        try {
          ...
          if (headerProj) {
            ...
            headerProj.addEventListener('click', (e) => { e.preventDefault(); try { showDbSelectModal(true); } catch (err) {} });
          }
        } catch (e) {}
  ...
  const ALLOW_HOVER_LABELS = !!(window && window.SIDEBAR_ALLOW_HOVER_LABELS);
        ...
        const ALLOW_SIDEBAR_EXPAND_ON_CLICK = !!(window && window.SIDEBAR_ALLOW_EXPAND_ON_CLICK);
        if (sidebar) {
          sidebar.addEventListener('click', (e) => {
            // ignore clicks on controls or project entries (entries handle their own clicks)
            if (!e.target || !e.target.closest) return;
            if (e.target.closest('.project-entry') || e.target.closest('button') || e.target.closest('.btn') || e.target.closest('a')) return;
            ...
            try { (window.setSidebarCollapsed || setSidebarCollapsed)(!collapsed, false); } catch (e) {}
          });
                ...
                projectListEl.addEventListener('mouseout', (e) => {
                  try {
                    ...
                    _sidebarHoverTimer = setTimeout(() => {
                      ...
                      try { updateSidebarLabelMode(); } catch (err) {}
              ...
              sidebar.addEventListener('mouseleave', (e) => {
                try {
                  ...
                  if (sidebar.classList.contains('expanded')) {
                    try { setSidebarCollapsed(true, false); } catch (err) {}
                  }
                  try { sidebar.classList.remove('hover-expanded'); } catch (err) {}
                  try { sidebar.classList.remove('no-sidebar-transition'); if (split) split.classList.remove('no-sidebar-transition'); } catch (err) {}
                  try { updateSidebarLabelMode(); } catch (e) {}
        ...
        try {
          ...
          try {
            ...
            if (wrap) {
              ...
              window.addEventListener('resize', () => {
                if (_lblResizeTimer) clearTimeout(_lblResizeTimer);
                _lblResizeTimer = setTimeout(() => { try { updateSidebarLabelMode(); } catch(e) {} }, 160);
              });
              if (typeof MutationObserver !== 'undefined') {
                const mo = new MutationObserver(() => { try { updateSidebarLabelMode(); } catch(e) {} });
                try { mo.observe(wrap, { childList: true }); } catch (e) {}
              }
            }
          } catch (e) {}
          ...
          try {
            window.setKbSidebarLabelCompactThreshold = function(n) {
              try { if (window && window.localStorage) localStorage.setItem('kbSidebarLabelCompactThreshold', String(n)); } catch(e) {}
              try { window.SIDEBAR_LABEL_COMPACT_THRESHOLD = n; } catch(e) {}
              try { updateSidebarLabelMode(); } catch(e) {}
            };
          } catch(e) {}
